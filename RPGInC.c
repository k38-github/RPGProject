            ...
            if ( SDL_PollEvent(&e) ) {
                if (e.type == SDL_KEYDOWN && e.key.keysym.sym == SDLK_SPACE) {
                    ...
                    while (*message != '\0') {
                        ...
                        if (strcmp(isspace, "  ") == 0) {
                           loop_counter = loop_counter + row_size - (loop_counter % row_size);
                        }
                        ...
                        if (strcmp(isasterisk, "**") == 0) {
                            loop_counter = loop_counter + col_size - (loop_counter % col_size);
                        }
                        ...
                        if (loop_counter % row_size == 0) {
                            display_character_string(renderer, font, message_out, 155 + col_position * 5 , 354 + row_position);
                            row_position += row_size;
                            col_position = 0;
                            tmp_position = word_length;
                            memset(message_out, '\0', 100);
